...
The task fails and lists each broken link with the page and line in
`_site/` it appears on. Run the internal check before pushing. The
external check makes a request for every outside URL, so it is slower
and can fail when a site is down or rate-limits the requests.
